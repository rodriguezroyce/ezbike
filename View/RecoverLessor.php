<?php
    ...
    if(isset($_POST["recover"])){
        ...
        if(empty($email)){
            ...
            $query = "SELECT * FROM `tbllessor` WHERE `lessor_email`='$email'";
            $db = new Database();
            $db->query($query);
            if($row = $db->fetchSingle()){
                $mailTo = $email;
                $subject = "Reset your Password";
                $msg = 'Hi '. $row["FirstName"] . ' Here is the link to reset your password http://localhost/ezbikerental/View/Reset_LessorPW.php?token='. $row["token"];
