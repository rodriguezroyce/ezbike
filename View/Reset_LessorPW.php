    ...
    if(isset($_GET['token'])){
        ...
        if(isset($_GET["reset"])){
            ...
            if(empty($password) || empty($confirmPassword)){
                ...
                $email = new Database();
                $emailQuery = "SELECT `lessor_email` FROM `tbllessor` WHERE `token`='$tokenFromUrl';";
                $email->query($emailQuery);
                $email->execute();
                $emailRow = $email->fetch();
                

                $query = "UPDATE `tbllessor` SET `lessor_password`='$hashPassword' WHERE `token`='$tokenFromUrl';";
                $db->query($query);
                if($db->execute()){
                    $mailTo = $emailRow["lessor_email"];
                    $subject = "Password Change";
                    $msg = 'Hi '. $emailRow["first_name"] . ' You have successfully change your password.';
                    $msg .= "To login page:  https://www.ezbikeofficial.com/View/lenderLogin.php";
